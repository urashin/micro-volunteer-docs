...
CREATE TABLE IF NOT EXISTS `volunteerdb`.`HandicapMaster` (
  `handicap_type` INT NOT NULL AUTO_INCREMENT,
  `handicap_name` VARCHAR(45) NOT NULL,
  `icon_path` VARCHAR(512) NOT NULL,
  `comment` VARCHAR(512) NULL,
  PRIMARY KEY (`handicap_type`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `volunteerdb`.`HandicapInfo` (
  ...
  INDEX `handicapped_id_idx` (`handicapped_id` ASC) VISIBLE,
  INDEX `HandicapInfo.handicap_type_idx` (`handicap_type` ASC) VISIBLE,
  CONSTRAINT `HandicapInfo.handicapped_id`
    FOREIGN KEY (`handicapped_id`)
    REFERENCES `volunteerdb`.`Users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `HandicapInfo.handicap_type`
